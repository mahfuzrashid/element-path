...
if (!class_exists('PB_Settings')) {
    class PB_Settings
    {
        ...
        function render_panel($args = array())
        {
            ...
            $_nav_footer = implode('', array_map(function ($footer_nav) {
                return sprintf('<a class="footer-link" href="%s" target="_blank">%s</a>',
                    ...
                    $this->get_data('label', esc_html__('Menu item', 'docpro'), $footer_nav)
                );
            }, $this->get_data('footer_menu', array(), $args)));
            $_nav_contents = sprintf('<ul class="meta-nav">%s</ul>', implode('', $nav_contents));

            printf('<div class="docpro-poll-meta"><div class="meta-sidebar">%s%s%s</div></div>', $_nav_header, $_nav_items, $_nav_footer);
        }
        ...
        function required_plugin_check()
        {
            ...
            foreach ($this->get_data('required_plugins', array()) as $this_plugin => $label) {

                $plugin_slug = explode('/', $this_plugin);
                $plugin_slug = isset($plugin_slug[0]) ? $plugin_slug[0] : '';
        ...
        function generate_gallery($option)
        {
            ...
            $sorting = isset($option['sorting']) ? (bool)$option['sorting'] : true;
            ...
            ?>
            <div class="<?php echo esc_attr($class); ?>" id="media_preview_<?php echo esc_attr($field_id); ?>">
                <?php echo $html; ?>
            </div>
            <div class='button <?php echo esc_attr($button_class); ?>' <?php echo esc_attr($disabled); ?>
                 id="media_upload_<?php echo esc_attr($field_id); ?>"><?php esc_html_e('Select Images'); ?></div>
            <script>
        ...
        function generate_datepicker($option)
        {
            ...
            wp_enqueue_style('jquery-ui', ELMPATH_PLUGIN_URL . 'assets/jquery-ui.css');
        ...
        function generate_timepicker($option)
        {
            ...
            wp_enqueue_style('jquery-ui-timepicker', ELMPATH_PLUGIN_URL . '/assets/jquery.timepicker.min.css');
            wp_enqueue_script('jquery-ui-timepicker', ELMPATH_PLUGIN_URL . '/assets/jquery.timepicker.min.js', array('jquery'));
        ...
        function generate_select($option)
        {
            ...
            $args = is_array($args) ? $args : $this->generate_args_from_string($args, $option);
            $value = isset($option['value']) ? $option['value'] : get_option($id);
            $placeholder = isset($option['placeholder']) ? $option['placeholder'] : esc_html__('Select option');
            $required = isset($option['required']) ? $option['required'] : false;
            $required = $required ? "required='required'" : '';
            $disabled = isset($option['disabled']) && $option['disabled'] ? 'disabled' : '';
            $data_attr = isset($option['data_attr']) && $option['data_attr'] ? $option['data_attr'] : '';
            ...
            <select <?php echo esc_attr($disabled); ?> <?php echo esc_attr($required); ?>
                    ...
                    id="<?php echo esc_attr($id); ?>" <?php echo esc_attr($data_attr); ?>>
